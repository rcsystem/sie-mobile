...
function deduplicarPorId(lista: any[]) {
  const mapa = new Map<any, any>();
  lista.forEach((p: any) => mapa.set(p.id_es, p));
  return Array.from(mapa.values());
}

function mensajeErrorAxios(e: any) {
  // timeout axios típico: e.code === 'ECONNABORTED' o mensaje incluye timeout
  if (e?.message?.toLowerCase?.().includes("timeout") || e?.code === "ECONNABORTED") {
    return "Tiempo de espera agotado (servidor no respondió)";
  }
  if (e?.name === "CanceledError" || e?.code === "ERR_CANCELED") {
    return "Solicitud cancelada";
  }
  return e?.response?.data?.message || "No se pudieron cargar permisos";
}
...
export default function PermisosListaScreen({ navigation }: any) {
  ...
  // Evita bucles de re-render por dependencias cambiantes en callbacks.
  // Usamos una ref como "fuente de verdad" para saber si estamos refrescando.
  const refrescandoRef = useRef(false);

  // Tabs mantienen la pantalla montada: si una request se cuelga, el spinner se queda.
  // Por eso usamos AbortController y apagamos estados al perder foco.
  const abortRef = useRef<AbortController | null>(null);
  const enPantallaRef = useRef(true);
  const primeraVezFocus = useRef(true);

  const pedirPagina = useCallback(async (pagina: number) => {
    // Cancela cualquier request anterior pendiente
    abortRef.current?.abort();
    const controller = new AbortController();
    abortRef.current = controller;

    const res = await listarPermisos(pagina, {
      signal: controller.signal,
      timeout: 12000, // ✅ evita que se quede colgada
    });

    const nuevos = res?.data ?? [];
    const hasMore = !!res?.meta?.has_more;
    return { nuevos, hasMore };
  }, []);
  ...
  const cargarInicial = useCallback(async () => {
    ...
    try {
      const { nuevos, hasMore } = await pedirPagina(1);

      if (!enPantallaRef.current) return;

      setPermisos(deduplicarPorId(nuevos));
      setHayMas(hasMore);
      setPage(2);
    } catch (e: any) {
      if (!enPantallaRef.current) return;
      const msg = mensajeErrorAxios(e);
      // si fue cancelación por navegación, no molestamos
      if (msg !== "Solicitud cancelada") setError(msg);
    } finally {
      if (enPantallaRef.current) setRefrescando(false);
  ...
  // ✅ Función estable: NO depende de `refrescando` (eso causaba loop en useFocusEffect)
  ...
  const cargarMas = useCallback(async () => {
    if (cargandoMas || refrescando || !hayMas) return;

    setCargandoMas(true);
    setError(null);

    try {
      const { nuevos, hasMore } = await pedirPagina(page);

      if (!enPantallaRef.current) return;

      setPermisos((prev) => deduplicarPorId([...prev, ...nuevos]));
      setHayMas(hasMore);
      setPage((p) => p + 1);
    } catch (e: any) {
      if (!enPantallaRef.current) return;
      ...
      if (enPantallaRef.current) setCargandoMas(false);
    }
  }, [cargandoMas, refrescando, hayMas, page, pedirPagina]);

  // ✅ Refresh al volver (pero no la primera vez para evitar doble carga)
  useFocusEffect(
    useCallback(() => {
      enPantallaRef.current = true;

      if (primeraVezFocus.current) {
        primeraVezFocus.current = false;
      } else {
        refrescar();
      }
      ...
      return () => {
        ...
        // Cancela request pendiente y apaga spinners sí o sí
        abortRef.current?.abort();
  ...
  return (
    <View style={{ flex: 1 }}>
      {error ? (
        <Text style={{ color: "#a10606", paddingHorizontal: 16, paddingTop: 10 }}>
          {error}
        </Text>
      ) : null}
      ...
      <FlatList
        ...
        ListFooterComponent={
          cargandoMas ? <ActivityIndicator style={{ marginVertical: 16 }} /> : null
        }
        renderItem={({ item }: any) => {
